...
function createNotification(type, title, message, targetRole = 'all', branchId = 'all') {
    ...
    // Trim to last 2000 to prevent unbounded memory growth
...
function hasRecentAlert(notifications, type, message, hours = 24) {
    ...
    return notifications.some(n =>
        n.type === type &&
        n.message === message &&
        moment(n.createdAt).isAfter(cutoff)
    );
}
...
function checkAlerts() {
    ...
    products.forEach(product => {
        ...
        // Low stock alert (deduplicated — max once per 24h per product)
        if (totalQty <= (product.minStock || settings.lowStockThreshold)) {
            const msg = `${product.name} ina stock chini (${totalQty} ${product.unit}). Tafadhali agiza zaidi.`;
            if (!hasRecentAlert(notifications, 'low_stock', msg)) {
                createNotification('low_stock', 'Low Stock Alert', msg, 'storekeeper');
            }
        }
        
        // Expiry alert (deduplicated — max once per 24h per batch)
        productStock.forEach(s => {
            if (s.expiryDate) {
                ...
                if (daysToExpiry <= settings.expiryAlertDays && daysToExpiry > 0) {
                    const msg = `${product.name} (Batch: ${s.batchNumber}) itaisha muda tarehe ${moment(s.expiryDate).format('DD/MM/YYYY')}`;
                    if (!hasRecentAlert(notifications, 'expiry', msg)) {
                        createNotification('expiry', 'Expiry Alert', msg, 'storekeeper');
                    }
                }
            }
        });
