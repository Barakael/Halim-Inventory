...
async function initApp() {
    ...
    const boot = document.getElementById('appBootStatus');
    if (boot) boot.style.display = 'block';

    // Always show a page section so login never lands on a blank white content area
    const dash = document.getElementById('page-dashboard');
    if (dash && !document.querySelector('.page-section.active')) {
    ...
    try {
        ...
        // Load initial data (do not block dashboard on failure)
        try { await loadCategories(); } catch (e) { console.error('loadCategories', e); }
        try { await loadBranches(); } catch (e) { console.error('loadBranches', e); }
        ...
        try { loadNotifications(); } catch (e) { console.error('loadNotifications', e); }
        ...
        // Keep shell visible; only redirect on clear auth failure
        window.location.replace('/login');
    } finally {
...
function getPageFromUrl() {
    ...
    let page = path.substring(1) || 'dashboard';
...
function navigateTo(page, updateUrl = true) {
    ...
    if (targetSection) {
        ...
        // Unknown route → show dashboard instead of blank content
